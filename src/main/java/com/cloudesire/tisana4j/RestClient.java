...
public class RestClient implements RestClientInterface {
	 ...
	 */
	public RestClient(String username, String password, boolean skipValidation,
			Map<String, String> headers, SSLContext ctx) {
		super();
	 ...
	 */
	@Override
	public void delete(URL url, Map<String, String> newHeaders)
			throws Exception {
	 ...
	 */
	@Override
	public <T> List<T> getCollection(URL url, Class<T> clazz,
			Map<String, String> newHeaders) throws Exception {
		HttpGet get = new HttpGet(url.toURI());
		setupMethod(get, newHeaders);
		HttpResponse response = execute(get);
		try {
			List<T> objList = mapper.reader(
	 ...
	 */
	@Override
	public void patch(URL url, Map<String, String> paramMap,
			Map<String, String> newHeaders) throws Exception {
	...
	public <T> T post(URL url, T obj, Map<String, String> newHeaders)
			throws Exception {
		return (T) post(url, obj, newHeaders, obj.getClass());
	}

	@Override
	public <T, R> R post(URL url, T obj, Map<String, String> newHeaders,
	...
	@Override
	public <T, R> R post(URL url, T obj, Map<String, String> newHeaders,
			Class<R> responseClass, Map<String, String> responseHeaders)
			throws Exception {
	 ...
	 */
	@Override
	public <T> T postData(URL url, String filename, InputStream content,
	...
	@Override
	public <T> T postData(URL url, String filename, InputStream content,
			Class<T> responseClass, Map<String, String> newHeaders)
			throws Exception {
		HttpPost post = new HttpPost(url.toURI());
	...
	@Override
	@SuppressWarnings("unchecked")
	public <T> T put(URL url, T obj, Map<String, String> newHeaders)
			throws Exception {
	...
	private void checkError(HttpResponse response) throws Exception {
		...
		if (httpResponseHandler != null) {
			httpResponseHandler.setResponse(responseCode, response
					.getStatusLine().getReasonPhrase());
		}
		if (responseCode < 200 || responseCode >= 300) {
			if (exceptionTranslator != null) {
				Exception translatedException = exceptionTranslator
						.translateError(responseCode, response.getStatusLine()
								.getReasonPhrase(), response.getEntity()
								.getContent());
				...
				throw new RestException(responseCode, response.getStatusLine()
						.getReasonPhrase());
		}
	}
	...
	private <T> T readObject(Class<T> clazz, HttpResponse response)
		...
		if (!useXml) {
			...
			@SuppressWarnings("unchecked")
			T obj = (T) unmarshallerB.unmarshal(response.getEntity()
					.getContent());
			return obj;
		}
	...
	private void setupMethod(HttpRequest request, Map<String, String> newHeaders) {
		...
		if (authenticated) {
			...
			String encoded = Base64Variants.MIME_NO_LINEFEEDS.encode((username
					+ ":" + password).getBytes());
	...
	private <T> void writeObject(T obj, HttpEntityEnclosingRequest request)
			throws IOException, JsonGenerationException, MappingException,
			JsonProcessingException, ParseException, JAXBException {
		if (!useXml) {
			try {
